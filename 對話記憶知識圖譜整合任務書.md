...
### 階段四：RAG 整合與知識圖譜應用 (RAG Query) ✅ 完成
...
* **驗收測試指令**（已修正使用 kg_id 強制路由）：
  ...
  ```
* **驗證結果**（2026-06-30 最終通過）：
  - ✅ KG 路由：`claude_memory` 正確被選中（score: 1.0）
  - ✅ 文件召回：`C--Users-666-Desktop-----__project_phase2_architecture`（score: 0.284）
  - ✅ LLM 生成：Ollama `HTTP/1.1 200 OK`，回傳完整繁體中文回答（confidence: 0.85）
* **修改紀錄**（已修改本機原始碼 + `docker cp` 進容器）：
  - `routers/agent.py`：新增 kg_id 強制路由邏輯，`_SIM_MIN_SCORE` 從 0.38 降至 0.28
  - `models/document.py`：`ChatRequest` 新增 `kg_id: UUID | None = None` 欄位
  - ✅ Image rebuild 已執行（2026-06-30），修改永久生效
* **Ollama 連線解法（已完成）**：
  - 以管理員身份執行 `netsh interface portproxy add v4tov4 listenaddress=0.0.0.0 listenport=11434 connectaddress=172.18.55.140 connectport=11434`
  - Docker 容器透過 `host.docker.internal:11434` 連到 WSL Ollama ✅
  - ⚠️ **注意**：portproxy 規則在 Windows 重開機後需重新執行（或設定為開機自動執行）
...
- [x] **#2 RAG 完整端到端驗證** ✅ 完成（2026-06-30）
  - [x] KG 路由驗證 ✅：`claude_memory` 正確被選中（score: 1.0）
  - [x] 文件召回驗證 ✅：文件被召回（score: 0.284）
  - [x] LLM 生成驗證 ✅：netsh portproxy 設定成功（管理員 UAC 提升），Ollama 200 OK
  - [x] portproxy 設定後：`docker restart kg-api` 已執行
  - [x] Image rebuild 已執行（background job）：
    ```powershell
